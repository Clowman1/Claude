...
export default class LeadDocumentRequestManager extends LightningElement {
    ...
    // Accepting a document says the file is legitimate, not that the request is satisfied. When
    // it is the last outstanding document the Loan Officer is asked whether to close the request
    // or keep it open, so a misfiled upload can be kept without clearing what was actually asked for.
    ...
    async acceptDocument(contentDocumentId, closeWhenNoneRemaining) {
        ...
        try {
            ...
            this.showToast(closeWhenNoneRemaining ? 'Document accepted and request cleared.' : 'Document accepted.');
        } catch (error) {
            this.showError(error);
        } finally {
            this.isLoading = false;
    ...
    async loadSelectedEmailTemplate() {
        ...
        this.emailToAddresses = renderedEmail?.toAddresses || this.borrowerEmail || '';
        this.emailCcAddresses = renderedEmail?.ccAddresses || '';
    ...
    // Documents still awaiting a decision. Drives the Close / Keep Open prompt on the last one.
    get pendingReviewDocumentCount() {
        return this.previewDocuments.filter(
            documentItem => (documentItem.reviewStatusLabel || 'Pending Review') === 'Pending Review'
        ).length;
    }
