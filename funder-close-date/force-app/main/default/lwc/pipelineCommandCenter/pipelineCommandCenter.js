...
export default class PipelineCommandCenter extends NavigationMixin(LightningElement) {
    ...
    decorateRows(tabKey, rows) {
        return (rows || []).map((row) => ({
            ...
            formattedClosingDate: this.formatDateTimeOrDate(null, row.closingDate),
    ...
    standardCellForColumn(row, column) {
        ...
        if (key === 'name') {
            ...
            value = row.formattedClosingDate || row.formattedDisplayDate || '';
            sortValue = row.closingDate || row.displayDate || row.displayDateTime || '';
