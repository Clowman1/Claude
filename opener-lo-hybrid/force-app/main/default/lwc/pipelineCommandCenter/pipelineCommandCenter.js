...
import logPastClientManualCall from '@salesforce/apex/PipelineCommandCenterController.logPastClientManualCall';
import logRealtorInPersonTouch from '@salesforce/apex/PipelineCommandCenterController.logRealtorInPersonTouch';
import logRealtorManualCall from '@salesforce/apex/PipelineCommandCenterController.logRealtorManualCall';
...
import searchReachEmployees from '@salesforce/apex/RealtorEventWorkspaceController.searchReachEmployees';
import addCoHosts from '@salesforce/apex/RealtorEventWorkspaceController.addCoHosts';
import addAllNickleyTeamInvitees from '@salesforce/apex/RealtorEventWorkspaceController.addAllNickleyTeamInvitees';
...
const LOAN_PARTNER_CHECKLIST_USER_IDS = new Set([
    '005Qi000007nP2rIAE', // Joey D'Alessandro
    '0055x00000BWkSBAA1'  // Megan Straight
]);
const RUNNING_TODO_USER_IDS = new Set([
    '005f2000009OtLnAAK'  // Christopher Lowman
]);
const MAX_SELECTED_COLUMNS = 14;
const LOCKED_FIRST_COLUMN_KEY = 'name';
...
const COMMAND_CENTER_COLUMNS = [
    ...
    { key: 'preApprovalDate', label: 'Pre-Approval Date', objects: ['Lead'] },
    { key: 'lastLoanOfficerRingCentralCall', label: 'Last LO RingCentral Call', objects: ['Lead'] },
    { key: 'lastLoanOfficerRingCentralSms', label: 'Last LO RingCentral SMS', objects: ['Lead'] },
    ...
    { key: 'loanCommitmentDate', label: 'Commitment Date', objects: ['Transaction__c'] },
    ...
    { key: 'loanOfficerName', label: 'LO', objects: ['Transaction__c'] },
...
export default class PipelineCommandCenter extends NavigationMixin(LightningElement) {
    ...
    isWebinarMarketingDashboard = false;
    isRhettDelaney = false;
    isRealtorEventHost = false;
    canCreateLead = false;
    ...
    editablePipelineObjectApiNames = [];
    canEditQuickButtons = true;
    isNickleyRealtorHost = false;
    canCreateTitleAgent = false;
    quickButtons = [];
    queueShortcuts = [];
    editableButtons = [];
    isRealtorEventModalOpen = false;
    realtorEventType;
    realtorEventName;
    realtorEventDate;
    realtorEventTime;
    realtorEventDuration = '60';
    realtorEventLocation;
    realtorEventNotes;
    isSavingRealtorEvent = false;
    realtorEventCoHostSearchTerm = '';
    realtorEventCoHostSearchResults = [];
    realtorEventCoHosts = [];
    realtorEventCoHostSearchTimer;
    inviteAllNickleyTeam = false;
    isTrackersDrawerOpen = false;
    columnPreferences = {};
    editableColumns = [];
    listViewOptions = [];
    error;
    ...
    wiredDashboard(result) {
        ...
        if (data) {
            ...
            this.isWebinarMarketingDashboard = data.isWebinarMarketingDashboard === true;
            this.isRhettDelaney = data.isRhettDelaney === true;
            this.isRealtorEventHost = data.isRealtorEventHost === true;
            ...
            this.editablePipelineObjectApiNames = data.editablePipelineObjectApiNames || [];
            this.canEditQuickButtons = data.canEditQuickButtons !== false;
            this.isNickleyRealtorHost = data.isNickleyRealtorHost === true;
            ...
            if (this.showJoeyMasterGroups) {
                ...
                this.selectedJoeyMasterGroupKey = activeJoeyGroupKey || this.selectedJoeyMasterGroupKey || (this.isRealtorEventHost ? 'nickleyLeads' : 'joeOlmsted');
            }
            ...
            this.editablePipelineObjectApiNames = [];
            this.canEditQuickButtons = true;
            this.isNickleyRealtorHost = false;
            ...
            this.isWebinarMarketingDashboard = false;
            this.isRhettDelaney = false;
            this.isRealtorEventHost = false;
        }
    }
    ...
    handleMasterGroupDrop(event) {
        ...
        this.saveMasterGroupOrder(this.isRealtorEventHost && droppedGroupType === 'joey' ? 'nickley' : droppedGroupType, groups);
        this.pipelineTabs = this.decorateTabs(this.pipelineTabs);
        this.clearDraggedMasterGroup();
    }
    ...
    openColumnEditor() {
        ...
        this.columnEditorSearchTerm = '';
        // Working order is edited locally and only saved when the user confirms, so Cancel cancels.
        this.columnEditorKeys = (this.activeDisplayColumns || []).map((column) => column.key);
    ...
    closeColumnEditor() {
        ...
        this.columnEditorKeys = [];
        this.columnEditorSearchTerm = '';
        this.columnEditorError = undefined;
    }

    handleColumnSearch(event) {
        this.columnEditorSearchTerm = event.target.value || '';
    }

    // Ordered list of what the table will show; top to bottom is left to right.
    get columnEditorShownColumns() {
        const availableByKey = new Map(
            this.availableColumnsForTab(this.activePipelineTab).map((column) => [column.key, column])
        );
        const keys = (this.columnEditorKeys || []).filter((key) => availableByKey.has(key));
        const lastIndex = keys.length - 1;
        const firstMovableIndex = keys[0] === LOCKED_FIRST_COLUMN_KEY ? 1 : 0;
        return keys.map((key, index) => {
            const column = availableByKey.get(key);
            const locked = key === LOCKED_FIRST_COLUMN_KEY;
            return {
                key,
                displayLabel: column.label,
                positionLabel: `${index + 1}`,
                isLockedColumn: locked,
                canRemove: !locked && !column.required,
                disableMoveUp: locked || index <= firstMovableIndex,
                disableMoveDown: locked || index >= lastIndex,
                moveUpLabel: `Move ${column.label} left`,
                moveDownLabel: `Move ${column.label} right`,
                requiredNote: column.required ? 'Required' : ''
            };
        });
    }

    get columnEditorAvailableColumns() {
        const shownKeys = new Set(this.columnEditorKeys || []);
        const term = (this.columnEditorSearchTerm || '').trim().toLowerCase();
        return this.availableColumnsForTab(this.activePipelineTab)
            .filter((column) => !shownKeys.has(column.key))
            .filter((column) => !term || String(column.label).toLowerCase().includes(term))
            .map((column) => ({ key: column.key, displayLabel: column.label }));
    }

    get hasColumnEditorAvailableColumns() {
        return this.columnEditorAvailableColumns.length > 0;
    }

    get columnEditorAtLimit() {
        return (this.columnEditorKeys || []).length >= MAX_SELECTED_COLUMNS;
    }

    get columnEditorLimitNote() {
        return this.columnEditorAtLimit
            ? `You are at the ${MAX_SELECTED_COLUMNS} column limit. Remove one to add another.`
            : '';
    }

    handleColumnEditorAdd(event) {
        const columnKey = event.currentTarget.dataset.key;
        if (!columnKey || (this.columnEditorKeys || []).includes(columnKey)) {
            return;
        }
        if (this.columnEditorAtLimit) {
            this.columnEditorError = `Choose up to ${MAX_SELECTED_COLUMNS} columns. Remove one before adding another.`;
            return;
        }
        this.columnEditorError = undefined;
        this.columnEditorKeys = [...this.columnEditorKeys, columnKey];
    }

    handleColumnEditorRemove(event) {
        const columnKey = event.currentTarget.dataset.key;
        this.columnEditorError = undefined;
        this.columnEditorKeys = (this.columnEditorKeys || []).filter((key) => key !== columnKey);
    }

    handleColumnEditorMoveUp(event) {
        this.moveColumnEditorKey(event.currentTarget.dataset.key, -1);
    }

    handleColumnEditorMoveDown(event) {
        this.moveColumnEditorKey(event.currentTarget.dataset.key, 1);
    }

    moveColumnEditorKey(columnKey, offset) {
        const keys = [...(this.columnEditorKeys || [])];
        const currentIndex = keys.indexOf(columnKey);
        const targetIndex = currentIndex + offset;
        const lowestAllowedIndex = keys[0] === LOCKED_FIRST_COLUMN_KEY ? 1 : 0;
        if (
            currentIndex < 0 ||
            columnKey === LOCKED_FIRST_COLUMN_KEY ||
            targetIndex < lowestAllowedIndex ||
            targetIndex >= keys.length
        ) {
            return;
        }
        keys.splice(currentIndex, 1);
        keys.splice(targetIndex, 0, columnKey);
        this.columnEditorError = undefined;
        this.columnEditorKeys = keys;
    }
    ...
    async saveColumnEditor() {
        const selectedKeys = (this.columnEditorKeys || []).filter(
    ...
    openRowEditor(event) {
        if (!this.canEditActivePipelineTabRows) {
            return;
        }
        ...
        const editableFieldsByApiName = new Map();
        (this.activeDisplayColumns || []).forEach((column) => {
            const cell = (row.customCells || []).find(
                (candidate) => candidate.rawKey === column.key || candidate.key === column.key
            );
            const fieldApiName = cell
                ? (cell.editable && cell.fieldApiName ? cell.fieldApiName : '')
                : this.fieldApiNameForColumn(column.key);
            if (fieldApiName) {
                editableFieldsByApiName.set(fieldApiName, {
                    key: `${row.id}-${fieldApiName}`,
                    fieldApiName,
                    label: column.displayLabel || column.label || fieldApiName
                });
            }
        });
        const editableFields = Array.from(editableFieldsByApiName.values());

        if (!editableFields.length) {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Nothing to edit',
                    message: 'None of the selected columns are editable for this record.',
                    variant: 'info'
                })
            );
            return;
    ...
    fieldApiNameForColumn(columnKey) {
        const fieldMapByObject = {
            Lead: {
                status: 'Status',
                phone: 'Phone',
                email: 'Email',
                appointmentDateTime: 'Appointment_Date_Time__c',
                leadSource: 'LeadSource',
                realtorBuyingAgent: 'Realtor_Buying_Agent__c',
                loanDetails: 'Loan_Details__c',
                loanPurpose: 'Loan_Purpose__c',
                loanAmount: 'Loan_Amount__c'
            },
            Transaction__c: {
                status: 'Status__c',
                closingDate: this.activePipelineTabKey === 'claFilesNotContacted24'
                    ? 'Conditionally_Approved_Date__c'
                    : 'Closing_Date__c',
                fundingDate: 'Funding_Date__c',
                processorName: 'Processor__c',
                detail: 'Processor_Note__c',
                lenderName: 'Lender_lookup__c',
                loanPurpose: 'Loan_Purpose__c',
                loanOfficerName: 'Loan_Officer__c',
                loanAmount: 'Loan_Amount_1st__c'
            },
            Account: {
                phone: 'Phone',
                email: 'PersonEmail',
        ...
        return fieldMapByObject[this.activePipelineObjectApiName]?.[columnKey] || '';
    }
    ...
    async handleRealtorManualCall(event) {
        const accountId = event.currentTarget.dataset.recordId;
        if (!accountId || this.manualCallSavingId) {
            return;
        }
        this.manualCallSavingId = accountId;
        try {
            await logRealtorManualCall({ accountId });
            // Reload the tab so My Last Call and the cadence colour both reflect the new call.
            this.pipelineTabs = this.pipelineTabs.map((tab) =>
                tab.key === 'myNickleyRealtors' ? { ...tab, rowsLoaded: false } : tab
            );
            await this.loadActiveNickleyRealtorTab();
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Call logged',
                    message: 'This Realtor is back in the normal contact rotation.',
                    variant: 'success'
                })
            );
        } catch (error) {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Unable to log call',
                    message: this.extractError(error),
                    variant: 'error'
                })
            );
        } finally {
            this.manualCallSavingId = undefined;
        }
    }

    async handleRealtorInPersonTouch(event) {
        const accountId = event.currentTarget.dataset.recordId;
        if (!accountId || this.inPersonTouchSavingId) {
            return;
        }
        this.inPersonTouchSavingId = accountId;
        try {
            await logRealtorInPersonTouch({ accountId });
            // Reload the tab so the stamp and the cadence colour both reflect the new touch.
            this.pipelineTabs = this.pipelineTabs.map((tab) =>
                tab.key === 'myNickleyRealtors' ? { ...tab, rowsLoaded: false } : tab
            );
            await this.loadActiveNickleyRealtorTab();
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Unable to log in person touch',
                    message: this.extractError(error),
                    variant: 'error'
                })
            );
        } finally {
            this.inPersonTouchSavingId = undefined;
        }
    }

    handlePastClientManualLog(event) {
        const accountId = event.currentTarget.dataset.recordId;
        if (!accountId) {
            return;
        }
        this.openPastClientConfirm({
            ...
            message:
                'This will log a completed outbound call from Ben on this Person Account and return the client to the normal call rotation.',
            buttonLabel: 'Log Manual Call'
        });
    }
    ...
    async confirmPastClientAction() {
        ...
        try {
            ...
            } else if (this.pastClientConfirmAction === 'manualLog') {
                await logPastClientManualCall({ accountId: this.pastClientConfirmRecordId });
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Manual call logged',
                        message: 'The outbound call was logged for Ben and the client has returned to the normal call rotation.',
                        variant: 'success'
                    })
                );
            } else {
    ...
    get rootClass() {
        const classes = ['pcc'];
        if (this.isCeoDashboard) {
            classes.push('pcc-ceo');
        }
        if (this.isWebinarMarketingDashboard) {
            classes.push('pcc-marketing');
        }
        if (this.isTrackersDrawerOpen) {
            classes.push('pcc-trackers-open');
        }
        return classes.join(' ');
    }

    get trackersDrawerClass() {
        return this.isTrackersDrawerOpen ? 'pcc-trackers-drawer pcc-trackers-drawer-open' : 'pcc-trackers-drawer';
    }

    get trackersToggleClass() {
        return this.isTrackersDrawerOpen ? 'pcc-trackers-toggle pcc-trackers-toggle-open' : 'pcc-trackers-toggle';
    }

    get trackersToggleLabel() {
        if (this.isLoanPartnerChecklistUser) {
            return this.isTrackersDrawerOpen ? 'Hide LP Daily Checklist' : 'LP Daily Checklist';
        }
        if (this.isRunningTodoUser) {
            return this.isTrackersDrawerOpen ? 'Hide To Do List' : 'To Do List';
        }
        return this.isTrackersDrawerOpen ? 'Hide trackers' : 'Press to view trackers';
    }

    get trackersDrawerTitle() {
        if (this.isLoanPartnerChecklistUser) return 'LP Daily Checklist';
        return this.isRunningTodoUser ? 'To Do List' : 'Trackers';
    }

    get trackersDrawerAriaLabel() {
        if (this.isLoanPartnerChecklistUser) return 'Loan Partner Daily Checklist';
        return this.isRunningTodoUser ? 'My To Do List' : 'Home trackers';
    }

    get isRunningTodoUser() {
        return RUNNING_TODO_USER_IDS.has(this.currentUserId);
    }

    get isLoanPartnerChecklistUser() {
        return LOAN_PARTNER_CHECKLIST_USER_IDS.has(this.currentUserId);
    }

    toggleTrackersDrawer() {
        this.isTrackersDrawerOpen = !this.isTrackersDrawerOpen;
    }
    ...
    get showStandardCommandCenter() {
        return !this.showCeoHubHomeOverride && !this.isWebinarMarketingDashboard;
    }
    ...
    get showRealtorEventActions() {
        // Scheduling events is for designated event hosts, which Apex already resolves as
        // isRealtorEventHost. Keying off the Nickley Realtor tab instead handed these buttons to
        // any LO with realtors assigned, whether or not they host events.
        return this.isRealtorEventHost;
    }

    createRealtorEvent(event) {
        this.realtorEventType = event.currentTarget.dataset.eventType;
        this.realtorEventName = `${this.realtorEventType} - ${new Date().toISOString().slice(0, 10)}`;
        this.realtorEventDate = new Date().toISOString().slice(0, 10);
        this.realtorEventTime = '';
        this.realtorEventCoHostSearchTerm = '';
        this.realtorEventCoHostSearchResults = [];
        this.realtorEventCoHosts = [];
        this.inviteAllNickleyTeam = false;
        this.isRealtorEventModalOpen = true;
    }

    handleRealtorEventFieldChange(event) {
        this[event.target.name] = event.target.value;
    }

    get realtorEventDurationOptions() {
        return [30, 60, 90, 120, 150, 180].map((minutes) => ({
            label: minutes < 60 ? '30 minutes' : `${minutes / 60}${minutes === 60 ? ' hour' : ' hours'}`,
            value: String(minutes)
        }));
    }

    get showRealtorEventCoHostSearchResults() {
        return this.realtorEventCoHostSearchTerm.trim().length >= 2;
    }

    get noRealtorEventCoHostSearchResults() {
        return this.showRealtorEventCoHostSearchResults && !this.realtorEventCoHostSearchResults.length;
    }

    handleRealtorEventCoHostSearch(event) {
        this.realtorEventCoHostSearchTerm = event.target.value;
        clearTimeout(this.realtorEventCoHostSearchTimer);
        if (this.realtorEventCoHostSearchTerm.trim().length < 2) {
            this.realtorEventCoHostSearchResults = [];
            return;
        }
        this.realtorEventCoHostSearchTimer = setTimeout(() => this.runRealtorEventCoHostSearch(), 250);
    }

    async runRealtorEventCoHostSearch() {
        try {
            const matches = await searchReachEmployees({ searchTerm: this.realtorEventCoHostSearchTerm });
            const selectedIds = new Set(this.realtorEventCoHosts.map((coHost) => coHost.userId));
            this.realtorEventCoHostSearchResults = (matches || []).filter((coHost) => !selectedIds.has(coHost.userId));
        } catch (error) {
            this.realtorEventCoHostSearchResults = [];
            this.dispatchEvent(new ShowToastEvent({ title: 'Unable to search co-hosts', message: this.extractError(error), variant: 'error' }));
        }
    }

    handleAddRealtorEventCoHost(event) {
        const userId = event.currentTarget.dataset.id;
        const coHost = this.realtorEventCoHostSearchResults.find((candidate) => candidate.userId === userId);
        if (!coHost) return;
        this.realtorEventCoHosts = [...this.realtorEventCoHosts, coHost];
        this.realtorEventCoHostSearchTerm = '';
        this.realtorEventCoHostSearchResults = [];
    }

    handleRemoveRealtorEventCoHost(event) {
        const userId = event.currentTarget.dataset.id;
        this.realtorEventCoHosts = this.realtorEventCoHosts.filter((coHost) => coHost.userId !== userId);
    }

    handleInviteAllNickleyTeamChange(event) {
        this.inviteAllNickleyTeam = event.target.checked;
    }

    closeRealtorEventModal() {
        if (!this.isSavingRealtorEvent) this.isRealtorEventModalOpen = false;
    }

    async saveRealtorEvent() {
        if (!this.realtorEventName || !this.realtorEventDate || !this.realtorEventTime || !this.realtorEventDuration) return;
        this.isSavingRealtorEvent = true;
        try {
            const startDate = this.realtorEventDate;
            const startDateTime = new Date(`${this.realtorEventDate}T${this.realtorEventTime}`);
            const endDateTime = new Date(startDateTime);
            endDateTime.setMinutes(endDateTime.getMinutes() + Number(this.realtorEventDuration));
            const endValue = endDateTime.toISOString();
            const endDate = endValue.slice(0, 10);
            const eventTime = `${this.realtorEventTime.slice(0, 5)}:00.000Z`;
            const campaign = await createRecord({ apiName: 'Campaign', fields: {
                Name: this.realtorEventName, Type: 'Event', Status: 'Planned', StartDate: startDate, EndDate: endDate,
                Event_Time__c: eventTime, Description: this.realtorEventNotes || ''
            }});
            await createRecord({ apiName: 'Event', fields: {
                Subject: this.realtorEventName, StartDateTime: startDateTime.toISOString(),
                EndDateTime: endValue, Location: this.realtorEventLocation || '',
                Description: this.realtorEventNotes || '', WhatId: campaign.id
            }});
            if (this.realtorEventCoHosts.length) {
                await addCoHosts({ campaignId: campaign.id, userIds: this.realtorEventCoHosts.map((coHost) => coHost.userId) });
            }
            if (this.inviteAllNickleyTeam) {
                await addAllNickleyTeamInvitees({ campaignId: campaign.id });
            }
            this.isRealtorEventModalOpen = false;
            this.dispatchEvent(new ShowToastEvent({ title: 'Realtor event created', message: 'Add invitees from Campaign Members.', variant: 'success' }));
            this[NavigationMixin.Navigate]({ type: 'standard__recordPage', attributes: { recordId: campaign.id, objectApiName: 'Campaign', actionName: 'view' } });
        } catch (error) {
            this.dispatchEvent(new ShowToastEvent({ title: 'Event was not created', message: this.extractError(error), variant: 'error' }));
        } finally { this.isSavingRealtorEvent = false; }
    }

    get showEditButtons() {
        // Nickley Realtor hosts receive a fixed quick button set, so the editor would let them
        // change preferences that are never read. Hide it rather than offer a no-op.
        return this.canEditQuickButtons && !this.isCeoDashboard && !this.isPastClientDashboard
            && !this.isNickleyRealtorHost;
    }
    ...
    isPreviewableCustomCell(cellKey, value) {
        ...
        const previewKeys = ['managerNotes', 'funderNote', 'detail', 'closerNotes', 'processorNote', 'actionRequiredReason', 'comments', 'notes'];
        return previewKeys.includes(cellKey) && value.length > 80;
    }

    customColumnLabel(tabKey, column) {
        if (tabKey === 'claFilesNotContacted24' && column.key === 'closingDate') {
            return 'Conditional Approval Date';
        }
        if (
    ...
    rowClass(tabKey, row) {
        ...
        if (tabKey === 'myNickleyRealtors' && row.callCadenceAlert === 'red') {
            classes.push('pcc-row-nickley-call-red');
        }
        if (tabKey === 'myNickleyRealtors' && row.callCadenceAlert === 'orange') {
            classes.push('pcc-row-nickley-call-orange');
        }
        return classes.join(' ');
    }
    ...
    get showJoeyMasterGroups() {
        return !this.isPastClientDashboard && (this.isRealtorEventHost || this.pipelineTabs.some((tab) => this.joeyGroupKeyForTab(tab.key)));
    }
    ...
    get activeJoeyMasterGroupKey() {
        if (this.taggedMasterGroups.length) {
            return this.selectedJoeyMasterGroupKey || this.joeyGroupKeyForTab(this.activePipelineTabKey) || this.taggedMasterGroups[0].key;
        }
        return this.selectedJoeyMasterGroupKey || this.joeyGroupKeyForTab(this.activePipelineTabKey) || (this.isRealtorEventHost ? 'nickleyLeads' : (this.usesMeganMasterGroups ? 'myLeads' : 'joeOlmsted'));
    }

    // Master groups declared by Apex on the tab itself (hybrid Opener/LO dashboards). Distinct
    // masterGroup values, in the order the tabs arrive.
    get taggedMasterGroups() {
        const groups = [];
        this.pipelineTabs.forEach((tab) => {
            if (tab.masterGroup && !groups.some((group) => group.key === tab.masterGroup)) {
                groups.push({ key: tab.masterGroup, label: tab.masterGroupLabel || tab.masterGroup });
            }
        });
        return groups;
    }
    ...
    get joeyMasterGroups() {
        const taggedGroups = this.taggedMasterGroups;
        if (taggedGroups.length) {
            const palette = ['pcc-joey-master-joe', 'pcc-joey-master-zach', 'pcc-joey-master-rhett', 'pcc-joey-master-berkeley'];
            return this.applySavedMasterGroupOrder(
                'joey',
                taggedGroups
                    .map((group, index) => this.buildJoeyMasterGroup(group.key, group.label, palette[index % palette.length]))
                    .filter((group) => group.hasTabs)
            );
        }
        if (this.isRealtorEventHost) {
            return this.applySavedMasterGroupOrder('nickley', [
                this.buildNickleyMasterGroup('nickleyLeads', 'Leads', 'Lead workflow queues.', 'pcc-joey-master-joe'),
                this.buildNickleyMasterGroup('nickleyTransactions', 'Transactions', 'Active file and closing workflow queues.', 'pcc-joey-master-zach'),
                this.buildNickleyMasterGroup('nickleyRealtors', 'Realtors', 'Nickley Realtor relationships and hosted events.', 'pcc-joey-master-rhett')
            ].filter((group) => group.hasTabs));
        }
    ...
    buildNickleyMasterGroup(key, label, subtitle, paletteClass) {
        const tabs = this.joeyTabsForGroup(key);
        const activeTab = tabs.find((tab) => key === 'nickleyLeads' ? /ActiveLeads$/.test(tab.key || '') : key === 'nickleyTransactions' ? /ActiveTransactions$/.test(tab.key || '') : tab.key === 'myNickleyRealtors');
        return {
            key,
            label,
            subtitle,
            count: Number(activeTab?.count) || tabs.reduce((total, tab) => total + (Number(tab.count) || 0), 0),
            hasTabs: tabs.length > 0,
            className: ['pcc-ben-master-tab', paletteClass, key === this.activeJoeyMasterGroupKey ? 'pcc-ben-master-active' : ''].filter(Boolean).join(' ')
        };
    }
    ...
    joeyGroupKeyForTab(tabKey) {
        const key = tabKey || '';
        // An Apex-declared master group wins over the per-user key-prefix rules below.
        const taggedTab = this.pipelineTabs.find((candidate) => candidate.key === key && candidate.masterGroup);
        if (taggedTab) {
            return taggedTab.masterGroup;
        }
        if (this.isRealtorEventHost) {
            if (['myNickleyRealtors', 'myRealtorEvents'].includes(key)) {
                return 'nickleyRealtors';
            }
            const tab = this.pipelineTabs.find((candidate) => candidate.key === key);
            return tab?.objectApiName === 'Transaction__c' ? 'nickleyTransactions' : 'nickleyLeads';
        }
        if (this.usesMeganMasterGroups) {
    ...
    get columnEditorCountText() {
        return `${(this.columnEditorKeys || []).length} / ${MAX_SELECTED_COLUMNS} selected`;
    }
    ...
    // Header cells get their own move arrows. Name is locked first, so it has none, and the
    // column immediately after it cannot move further left.
    get activeDisplayColumnsWithControls() {
        const columns = this.activeDisplayColumns || [];
        const lastIndex = columns.length - 1;
        const firstMovableIndex = columns[0]?.key === LOCKED_FIRST_COLUMN_KEY ? 1 : 0;
        return columns.map((column, index) => {
            const locked = column.key === LOCKED_FIRST_COLUMN_KEY;
            return {
                ...column,
                showMoveControls: !locked && columns.length > 1,
                disableMoveLeft: index <= firstMovableIndex,
                disableMoveRight: index >= lastIndex,
                moveLeftLabel: `Move ${column.displayLabel} left`,
                moveRightLabel: `Move ${column.displayLabel} right`
            };
        });
    }

    handleColumnMoveLeft(event) {
        this.moveActiveColumn(event.currentTarget.dataset.key, -1);
    }

    handleColumnMoveRight(event) {
        this.moveActiveColumn(event.currentTarget.dataset.key, 1);
    }

    async moveActiveColumn(columnKey, offset) {
        const keys = (this.activeDisplayColumns || []).map((column) => column.key);
        const currentIndex = keys.indexOf(columnKey);
        const targetIndex = currentIndex + offset;
        const lowestAllowedIndex = keys[0] === LOCKED_FIRST_COLUMN_KEY ? 1 : 0;
        if (
        ...
        keys.splice(currentIndex, 1);
        keys.splice(targetIndex, 0, columnKey);
        await this.persistColumnOrder(keys);
    }

    async persistColumnOrder(keys) {
        const nextPreferences = {
            ...this.columnPreferences,
            [this.activePipelineTabKey]: keys
        };
        // Reflect the move immediately so the header does not lag behind the click.
        this.columnPreferences = nextPreferences;
        this.pipelineTabs = this.decorateTabs(this.pipelineTabs);
        try {
            const savedPreferences = await saveUserColumnPreferences({
                columnPreferencesJson: JSON.stringify(nextPreferences)
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Unable to save column order',
                    message: this.extractError(error),
                    variant: 'error'
                })
            );
    ...
    selectedColumnsForTab(tab) {
        ...
        let requestedKeys = Array.isArray(this.columnPreferences?.[tab.key])
            ? this.columnPreferences[tab.key]
            : defaultKeys;
        // Approaching Commitments is organised around the commitment date, so that column always
        // shows and Owner never does. Applied to saved layouts too, otherwise users who already
        // customised the tab never receive the change. filter() copies first, so the stored
        // preference array is never mutated in place.
        if (tab.key === 'approachingCommitments') {
            requestedKeys = requestedKeys.filter((key) => key !== 'ownerName');
            if (!requestedKeys.includes('loanCommitmentDate')) {
                const closingDateIndex = requestedKeys.indexOf('closingDate');
                if (closingDateIndex === -1) {
                    requestedKeys.push('loanCommitmentDate');
                } else {
                    requestedKeys.splice(closingDateIndex, 0, 'loanCommitmentDate');
                }
            }
        }
        // Funder Action Required swapped Manager Notes for Funder Note. A saved layout still lists
        // the retired key, which no longer resolves, so the note column would silently disappear.
        if (tab.key === 'funderActionRequired') {
            const retiredIndex = requestedKeys.indexOf('managerNotes');
            requestedKeys = requestedKeys.filter((key) => key !== 'managerNotes');
            if (!requestedKeys.includes('funderNote')) {
                if (retiredIndex === -1) {
                    requestedKeys.push('funderNote');
                } else {
                    requestedKeys.splice(Math.min(retiredIndex, requestedKeys.length), 0, 'funderNote');
                }
            }
        }
        const requiredKeys = availableColumns.filter((column) => column.required).map((column) => column.key);
        if (tab.key === 'myNickleyRealtors') {
            requiredKeys.push('nextEvent', 'myLastCall', 'myLastText', 'myLastInPersonTouch', 'notes');
        }
        if (tab.key === 'stagnantPreApprovals') {
            requiredKeys.push('preApprovalDate', 'lastLoanOfficerRingCentralCall', 'lastLoanOfficerRingCentralSms');
        }
        // Required columns must be present, but appending only the missing ones keeps them from
        // jumping to the front and overriding an order the user deliberately chose.
        const missingRequiredKeys = requiredKeys.filter((key) => !requestedKeys.includes(key));
        const selectedKeys = this.orderSelectedColumnKeys(tab, [...requestedKeys, ...missingRequiredKeys])
    ...
    // Only Name is pinned, because it carries the record link users scan by. Everything else -
    // including LO / Owner - follows whatever order the user saved, so the move arrows stick.
    orderSelectedColumnKeys(tab, keys) {
        const orderedKeys = [];
        if ((keys || []).includes(LOCKED_FIRST_COLUMN_KEY)) {
            orderedKeys.push(LOCKED_FIRST_COLUMN_KEY);
        }
        (keys || []).forEach((key) => {
    ...
    defaultColumnKeysForTab(tab) {
        ...
        if (objectApiName === 'Transaction__c') {
            const columns = [
                ...
                'status',
                ...(tabKey === 'approachingCommitments' ? ['loanCommitmentDate'] : []),
                'closingDate',
                'detail'
            ];
            ...
            if (!['assigned', 'filesInClosing', 'approachingCommitments'].includes(tabKey)) {
                columns.push('ownerName');
            }
            columns.push('loanAmount', 'easeLink');
            return columns;
        }
    ...
    availableColumnsForTab(tab) {
        ...
        (tab.customColumns || []).forEach((column) => {
            columnsByKey.set(column.key, {
                ...
                required: this.isRequiredColumnKey(objectApiName, column.key)
            });
        });
    ...
    isRequiredColumnKey(objectApiName, key) {
        return (
            key === 'name' ||
            (objectApiName === 'Lead' && key === 'ownerName') ||
            (objectApiName === 'Transaction__c' && key === 'loanOfficerName')
        );
    }
    ...
    standardCellForColumn(row, column) {
        ...
        } else if (key === 'preApprovalDate') {
            value = this.formatDateTimeOrDate(null, row.preApprovalDate);
            sortValue = row.preApprovalDate || '';
        } else if (key === 'lastLoanOfficerRingCentralCall') {
            value = this.formatDateTimeOrDate(row.lastLoanOfficerRingCentralCall, null);
            sortValue = row.lastLoanOfficerRingCentralCall || '';
        } else if (key === 'lastLoanOfficerRingCentralSms') {
            value = this.formatDateTimeOrDate(row.lastLoanOfficerRingCentralSms, null);
            sortValue = row.lastLoanOfficerRingCentralSms || '';
        } else if (key === 'loanCommitmentDate') {
            value = this.formatDateTimeOrDate(null, row.loanCommitmentDate);
            sortValue = row.loanCommitmentDate || '';
    ...
    get customTableClass() {
        ...
        return 'pcc-table pcc-compact-custom-table';
    }
    ...
    get canEditActivePipelineTabRows() {
        return (
            this.canEditPipelineTabRows &&
            this.editablePipelineObjectApiNames.includes(this.activePipelineObjectApiName)
        );
    }
    ...
    get isNickleyRealtorTab() {
        return this.activePipelineTabKey === 'myNickleyRealtors';
    }

    // The assigned Realtor list logs in person conversations instead of opening the row editor.
    get showRealtorEditColumn() {
        return this.canEditActivePipelineTabRows && !this.isNickleyRealtorTab;
    }
